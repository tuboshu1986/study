...
var ManagerGlobalObjectUtilsType = (function(){
	var Type = function(){};
	
    /**
     * 判断对象的所有的基本值（不包括Object、Array等）是否都为空，遍历所有层
     * @param from
     * @param to
     */
	Type.prototype.propsIsEmpty = function(obj){
    ...
    /**
     * 判断对象的所有的基本值（不包括Object、Array等）是否都不为空，遍历所有层
     * @param from
     * @param to
     */
	Type.prototype.propsIsNotEmpty = function(obj){
		if(!obj || typeof obj != "object"){
			return false;
		}
		for(let key in obj){
			let innerObj = obj[key];
			if(typeof innerObj == "object"){
				if(this.propsIsNotEmpty(innerObj))
					return false;
			}else if(!innerObj){
				return false;
			}
...
var ManagerGlobalObjectUtils = (function(){
	return new ManagerGlobalObjectUtilsType();
})();